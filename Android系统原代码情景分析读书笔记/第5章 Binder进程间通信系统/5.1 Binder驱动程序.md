...
31 
32 		BUG_ON(*page);
33 		*page = alloc_page(GFP_KERNEL | __GFP_ZERO);// 真正进行内存的分配
34         ......
35         
36 		tmp_area.addr = page_addr;
37 		tmp_area.size = PAGE_SIZE + PAGE_SIZE /* guard page? */;
38 		page_array_ptr = page;
39 		ret = map_vm_area(&tmp_area, PAGE_KERNEL, &page_array_ptr);// 在内核空间进行内存映射
40         ......
41         
42 		user_page_addr =
43 			(uintptr_t)page_addr + proc->user_buffer_offset;
44 		ret = vm_insert_page(vma, user_page_addr, page[0]);// 在用户空间进行内存映射
45         ......
46 
47 	}
48 	if (mm) {
49 		up_write(&mm->mmap_sem); //释放内存的写信号量
50 		mmput(mm);//减少mm->mm_users计数
51 	}
52 	return 0;
53 
54 free_range: //释放内存的流程
...
```cpp
30 	while (n) { //从binder_buffer的红黑树中查找大小相等的buffer块
...
```cpp
54 	has_page_addr =
55 		(void *)(((uintptr_t)buffer->data + buffer_size) & PAGE_MASK);//计算空闲内核缓冲区 buffer 的结束地址所在的页面的起始地址
56 	if (n == NULL) {
...
61 	}
62 	end_page_addr =
63 		(void *)PAGE_ALIGN((uintptr_t)buffer->data + buffer_size);//结束地址对齐到页面边界
...
调整好变量 `free_page_start` 和 `free_page_end` 的值后，第 `34` 行首先将 `binder_buffer` 结构体 `buffer` 所描述的内核缓冲区从目标进程 `proc` 的内核缓冲区列表中删除，接着第 `35` 行的 `if` 语句检查是否需要释放 `binder_buffer` 结构体 `buffer` 所占用的物理页面。如果变量 `free_page_start` 或者 `free_page_end` 的值有一个等于 `1` ，那么第 `32` 行就需要调用函数 `binder_update_page_range` 来释放它所在的虚拟地址页面所对应的物理页面。变量 `free_page_start` 、 `free_page_end` 的值与要释放的物理页面的对应关系如表 `5-1` 所示。
